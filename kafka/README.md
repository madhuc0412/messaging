...
```
cd .\messaging\kafka\
docker build . -t madhuc/kafka:2.7.0
...
```
docker run --rm --name kafka -it madhuc/kafka:2.7.0 bash
...
```
cd ./zookeeper
docker build . -t madhuc/zookeeper:2.7.0
cd ..
```
